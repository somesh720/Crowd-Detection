...
model = YOLO("v11l.onnx", task="detect")
...
def download_youtube_video_only(url, output_path="temp_video.mp4"):
    ydl_opts = {
...
def fetch_media_from_url(url):
    try:
        ...
        if youtube_id:
            ...
            video_path = download_youtube_video_only(url)
...
def show_image_preview():
    image = st.session_state.media.convert("RGB")
    ...
    for frame in video.decode(video=0):
        ...
        for box, cls in zip(results.boxes.xyxy, results.boxes.cls):
            if int(cls.item()) == 0:  # Person class
                ...
                dets.append(([x1, y1, x2 - x1, y2 - y1], 0.99, 'person'))
        ...
        for track in tracks:
            ...
            cv2.rectangle(img, (int(l), int(t)), (int(w), int(h)), (0, 0, 255), 2)
            cv2.putText(img, f'ID: {track_id}', (int(l), int(t) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)

        stframe.image(cv2.cvtColor(img, cv2.COLOR_BGR2RGB), use_container_width=True)

    count_placeholder.markdown(
        f"<div style='text-align: center; font-size: 24px; font-weight: bold; color: white;'>👥 Unique People Detected in Video: <span style='color: #00ffff;'>{len(unique_ids)}</span></div>",
        unsafe_allow_html=True
    )
...
# Optional: YouTube Preview (disabled)
def show_youtube_preview():
    st.error("Processing not available for embedded YouTube. Please use a direct video link.")
    if st.button("Try Another Image or Video"):
        reset_app_state()
...
if st.session_state.page == "upload":
    ...
    if st.session_state.media_type == "image":
        ...
        show_video_preview()
    elif st.session_state.media_type == "youtube":
        show_youtube_preview()
