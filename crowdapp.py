...
def fetch_media_from_url(url):
    try:
        ...
        parsed_url = urlparse(url)  # ✅ Fix here
        if any(parsed_url.path.lower().endswith(ext) for ext in ["jpg", "jpeg", "png", "avif", "webp"]):
            img = fetch_image_from_url(url)
            st.session_state.media = img
            st.session_state.media_type = "image"
        elif any(parsed_url.path.lower().endswith(ext) for ext in ["mp4", "mov", "avi"]):
...
def show_video_preview():
    ...
    count_placeholder.markdown(
        f"<div style='text-align: center; font-size: 24px; font-weight: bold; color: white;'>👥 Estimated Total People in Video: <span style='color: #00ffff;'>{max_people_detected}</span></div>",
        unsafe_allow_html=True
    )
